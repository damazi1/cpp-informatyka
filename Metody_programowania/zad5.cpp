...
void polygon::read(ifstream &f)
{
  ...
  if (buf != "[POLYGON]")
    throw string("Brak nagłówka \"[POLYGON]\"");
  // cout << buf << endl;

  getline(f, buf);
  if (buf != "[NUMBER OF NODES]")
  ...
  if (n < 3)
    throw string("Liczba punktów mniejsza od 3");

  getline(f, buf);
  if (buf != "[NODES]")
    throw string("Brak nagłówka \"[NODES]\"");
  // cout << buf << endl;
  points = new point[n];
...
double polygon::obwod() const
{
  double obw = 0;
  if (!order)
    throw string("Brak tablicy order.");
  if (!points)
    throw string("Brak tablicy points.");
  for (int i = 0, j = 0; (unsigned)i < n; ++i)
  {
    ...
    if (j > n)
      throw string("Niedozwolona wartość >n w tablicy order");
...
double polygon::pole() const
{
  ...
  for (size_t i = 0; i < n ; i++)
  {
    suma+=points[i].distance(points[order[(i + 1) % n] - 1]);
  }
  return suma / 2;
}
...
catch (int err)
{
  ...
  case 1:
    ...
    break;
  case 2:
    cout << err << ": Złe rozszerzenie pliku z danymi - musi być *.txt .\n";
    break;
  case 3:
